...
//	MAIN CALCULATION 
	...
	while($row = mysqli_fetch_array($retval,MYSQL_ASSOC))
	{
		...
		while($row2 = mysqli_fetch_array($retval2,MYSQL_ASSOC))
		{
			...
			array_push($ia,marks($row2['ia']));
			array_push($iam,marks($row2['iam']));
			array_push($iap,marks($row2['iap']));
			array_push($iapm,$row2['iapm']);
			array_push($ue,marks($row2['ue']));
			...
			/*if($row2['uem'] == 0)
				...
				array_push($values,absent($row2['ue'])."/".marks($row2['uem']));
			...
			if($row2['iam'] == 0)
				...
				array_push($values, $row2['ia']."/".$row2['iam']);


			$total = marks($row2['ue'])+marks($row2['uep'])+marks($row2['ia']);
			$totalm = marks($row2['uem'])+marks($row2['uepm'])+marks($row2['uem']);
