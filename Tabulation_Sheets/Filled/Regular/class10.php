...
while( $row = mysqli_fetch_array($retval, MYSQL_ASSOC))
{
		...
		{		
			...
			array_push($ROW1,absent($row2['ia']));
			array_push($ROW1,marks($row2['ue'])+marks($row2['ia'])."/".grade(marks($row2['ue'])+marks($row2['ia']),60));
			array_push($ROW1,$total);
			//ROW 2
			array_push($ROW2,"");
			array_push($ROW2,absent($row2['ue']));
			array_push($ROW2,absent($row2['uep'])."/".grade($row2['uep'],40));
			array_push($ROW2,grade($total,100)."/".grade_point($total,100));
			
			$total_marks += marks($row2['ue'])+marks($row2['ia'])+marks($row2['iap'])+marks($row2['uep']);
		}
		
		$remark = remarks($fa,$sa1,$sa2,$code);
		array_push($ROW1,$total_marks);
		array_push($ROW1,$remark);
		array_push($ROW2,grade_point($total_marks,700));
		array_push($ROW2,"");
