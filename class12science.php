...
$q = "select distinct sid, sname, fname, roll, enroll, cat, dob from class12 where private='no' and stream='science'";
$retval = mysqli_query($conn,$q);
while($row = mysqli_fetch_array($retval,MYSQL_ASSOC))
{
	$s = "select s.subname as name, s.code as code, c.ia as 
	ia, c.ue as ue, c.uep as uep, c.iap as iap, s.ia as iam, c.sauth as auth, s.ue as uem, s.uep as uepm, s.iap as iapm, c.sauth as auth from class12 c, subject12 as s where s.code = c.subject_code and c.private = s.private and c.private = 'no' and sid = ".$row['sid'];
...
$pdf->Line(10,$pdf->GetY(),200,$pdf->GetY());
$pdf->Cell(0,2,"",0,1);
$qr = "Roll No: ".$row['roll']." | Enrollment: ".$row['enroll']." | Name: ".$row['sname']." | Father's name : ".$row['fname']." | DOB: ".$row['dob']." | Total Marks: $max / 500";
include('phpQR.php');
genQR($row['sid'],$qr);
$QR = $tempQR.$row['sid'].'.png';
$pdf->Cell(175,5,"TOTAL    $max / 500",0,1,'R');
$pdf->Cell(0,2,"",0,1);
$pdf->Line(10,$pdf->GetY(),200,$pdf->GetY());
$pdf->Cell(0,3,"",0,1);
$pdf->Cell(0,5,"RESULT / REMARKS    ".remarks($ia,$ue,$iap,$uep,$iam,$uem,$iapm,$uepm,$code),0,1);
$pdf->Cell(0,3,"",0,1);
$pdf->Line(10,$pdf->GetY(),200,$pdf->GetY());
$pdf->Ln();
$pdf->Cell(0,15,"","",1);
