...
$q = "select distinct sid, sname, fname, roll, enroll, cat, dob from class12 where private = 'yes'";
$retval = mysqli_query($conn,$q);
$iter = 0;
while($row = mysqli_fetch_array($retval,MYSQL_ASSOC))
{
	
	$s = "select s.subname as name, c.sauth as auth, s.code as code, c.ia as ia, c.ue as ue, c.uep as uep, c.iap as iap, s.ia as iam, s.ue as uem, s.uep as uepm, s.iap as iapm from class12 c, subject12 as s where s.code = c.subject_code and s.private = c.private and c.sid = ".$row['sid']." and c.private = 'yes'";//" and c.total > 0";
...
$pdf->Output();
//$pdf->Output("Marksheet-XII-".date("d-m-Y").".pdf","F");
